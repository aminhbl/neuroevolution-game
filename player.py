...
class Player(pygame.sprite.Sprite):
    def __init__(self, game_mode):
        ...
        if self.game_mode == "Neuroevolution":
            ...
            layer_sizes = [8, 12, 6, 1]  # TODO (Design your architecture here by changing the values)
    ...
    def think(self, screen_width, screen_height, obstacles, player_x, player_y):
        ...
        input_vec = create_input_smart(screen_width, screen_height, obstacles, player_x, player_y)

        output_vec = self.nn.forward(input_vec)

        if output_vec[0, 0] >= 0.5:
...
def create_input(screen_width, screen_height, obstacles, player_x, player_y):
    input_vec = np.zeros((8, 1))

    input_vec[0, 0] = player_x / screen_width
    input_vec[1, 0] = player_y / screen_height

    if len(obstacles) > 0:
        obs1 = obstacles[0]
        input_vec[2, 0] = obs1['x'] / screen_width
        input_vec[3, 0] = max(obs1['y'] / screen_height, 0)

    if len(obstacles) > 1:
        obs2 = obstacles[1]
        input_vec[4, 0] = obs2['x'] / screen_width
        input_vec[5, 0] = max(obs2['y'] / screen_height, 0)

    if len(obstacles) > 2:
        obs3 = obstacles[2]
        input_vec[6, 0] = obs3['x'] / screen_width
        input_vec[7, 0] = max(obs3['y'] / screen_height, 0)

    return input_vec


def create_input_smart(screen_width, screen_height, obstacles, player_x, player_y):
    input_vec = np.zeros((8, 1))
    ...
    if len(obstacles) > 0:
        obs1 = obstacles[0]
        if obs1['x'] == 177 or obs1['x'] == 430:
            input_vec[2, 0] = obs1['x'] / screen_width
            input_vec[3, 0] = max(obs1['y'] / screen_height, 0)
            input_vec[4, 0] = 0

        elif obs1['x'] <= (177 + 430) / 2:
            input_vec[2, 0] = 177 / screen_width
            input_vec[3, 0] = max(obs1['y'] / screen_height, 0)
            input_vec[4, 0] = np.abs(177 - obs1['x']) / screen_width

        else:
            input_vec[2, 0] = 430 / screen_width
            input_vec[3, 0] = max(obs1['y'] / screen_height, 0)
            input_vec[4, 0] = np.abs(430 - obs1['x']) / screen_width

    if len(obstacles) > 1:
        obs2 = obstacles[1]
        if obs2['x'] == 177 or obs2['x'] == 430:
            input_vec[5, 0] = obs2['x'] / screen_width
            input_vec[6, 0] = max(obs2['y'] / screen_height, 0)
            input_vec[7, 0] = 0

        elif obs2['x'] <= (177 + 430) / 2:
            input_vec[5, 0] = 177 / screen_width
            input_vec[6, 0] = max(obs2['y'] / screen_height, 0)
            input_vec[7, 0] = np.abs(177 - obs2['x']) / screen_width

        else:
            input_vec[5, 0] = 430 / screen_width
            input_vec[6, 0] = max(obs2['y'] / screen_height, 0)
            input_vec[7, 0] = np.abs(430 - obs2['x']) / screen_width
